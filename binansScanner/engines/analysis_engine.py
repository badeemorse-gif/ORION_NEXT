...
class AnalysisEngine:
    ...
    DEFAULT_TIMEFRAME_ORDER: tuple[Timeframe, ...] = (
        Timeframe.H1, Timeframe.H4, Timeframe.M15, Timeframe.D1, Timeframe.M1, Timeframe.M5,
    )

    def __init__(self, default_timeframe: Optional[Timeframe | str] = None) -> None:
    ...
    def _select_primary_timeframe(self, dataset: MarketDataset) -> tuple[Optional[TimeframeData], Optional[Timeframe]]:
    ...
    @staticmethod
    def _normalize_timeframe(timeframe: Optional[Timeframe | str]) -> Optional[Timeframe]:
    ...
    def _missing_required_indicators(self, dataframe: pd.DataFrame) -> list[str]:
        return [indicator for indicator in self.REQUIRED_INDICATORS if indicator not in dataframe.columns]
    ...
    def _determine_market_state(self, dataframe: pd.DataFrame, signals: list[str]) -> str:
        ...
        ema_9, ema_20, ema_50 = last_row["ema_9"], last_row["ema_20"], last_row["ema_50"]
    ...
    def _calculate_market_strength(self, dataframe: pd.DataFrame, signals: list[str]) -> float:
        ...
        if "momentum_5" in dataframe.columns and "close" in dataframe.columns:
            ...
            if not pd.isna(momentum) and not pd.isna(close) and float(close) > 0.0:
                ...
                if relative_momentum > 0: signals.append("MOMENTUM_POSITIVE")
                elif relative_momentum < 0: signals.append("MOMENTUM_NEGATIVE")
