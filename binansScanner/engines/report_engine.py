...
class ReportEngine:
    ...
    def build_report(
        ...
        *,
        ...
        expected = ((analysis, AnalysisResult, "analysis"), (profile, ProfileResult, "profile"), (score, ScoreResult, "score"), (decision, DecisionResult, "decision"), (execution, ExecutionResult, "execution"))
        ...
        if not stage_trace:
            stage_trace = tuple(name for present, name in ((analysis is not None, "ANALYSIS"), (profile is not None, "PROFILE"), (score is not None, "SCORE"), (decision is not None, "DECISION"), (execution is not None, "EXECUTION")) if present)
        ...
        audit = ReportAudit(status=audit_status, stage_trace=tuple(str(item) for item in stage_trace), decision_reasons=decision_reasons, decision_warnings=decision_warnings, execution_status=execution_status, execution_message=execution_message, order_id=order_id, failure_stage=str(failure_stage) if failure_stage is not None else None, failure_message=resolved_failure_message)
        ...
        return ReportResult(symbol=symbol.strip(), analysis=analysis, profile=profile, score=score, decision=decision, execution=execution, summary=tuple(str(item) for item in summary), highlights=tuple(str(item) for item in highlights), warnings=normalized_warnings, metadata=ReportMetadata(project_version=self._project_version, report_name=report_name, execution_time_ms=max(float(execution_time_ms), 0.0)), audit=audit)
