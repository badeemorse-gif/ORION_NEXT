...
## 13. Paper Capital-Account Simulation Gate

After the Paper Bot passes the intelligence-accuracy, real-time, order-lifecycle, and six-hour stability gates, it shall run a **capital-account simulation** with a fixed virtual starting balance (for example, `$200`) for a controlled **24-hour** period before any Real Bot authorization.

This simulation is an accounting and behavioral-rehearsal gate, not a profitability guarantee and not a live-trading test. No real funds or live orders are permitted.

The simulation shall maintain an explicit virtual account containing at minimum:

- starting cash/equity,
- available cash,
- open positions,
- pending orders,
- position sizing,
...
- realized P&L,
- unrealized P&L,
- fees and configured slippage assumptions,
- equity curve,
- maximum drawdown,
- ending equity,
- all order/position lifecycle events.

The 24-hour report shall reconcile every fill and exit against the virtual ledger and prove:

1. No balance is created or destroyed outside the ledgered events and configured costs.
2. No duplicate fills or duplicate positions occur.
3. Pending orders are cancelled/replaced according to the stale-signal rules.
4. Stop-loss, take-profit, reversal, and risk exits are reflected correctly in realized P&L.
5. The ending equity equals starting equity plus reconciled realized/unrealized P&L after configured costs.
6. The full event trail is sufficient to reconstruct the account state at any point in the 24-hour run.

The output must report at least:

`STARTING_EQUITY → ENDING_EQUITY → NET_P&L → MAX_DRAWDOWN → TOTAL_TRADES → WIN_RATE → FEES/SLIPPAGE → OPEN_POSITIONS_AT_END`

The 24-hour capital-account simulation is a **mandatory release gate** before Real Bot authorization. Its results must not be interpreted as proof that the strategy will be profitable in future live markets.
...
- no reconciled 24-hour virtual-capital simulation,
