...
class TestPaperExperimentReport(unittest.TestCase):
    def test_report_contains_complete_contract_without_recomputing_accounting(self):
        ...
        observation = PaperExperimentObservation(
            opportunity_evaluated=4,
            opportunity_accepted=2,
            entry_evaluated=2,
            entry_accepted=1,
            actionable_outcomes=1,
            false_negatives=0,
            strategy_rejections=1,
            ...
            committed_capital_samples=(5.0,),
            closed_trade_pnl=(1.0,),
            hold_seconds=(120.0,),
        )
        report = report_from_runner(config=config, runner_report=runner_report, observation=observation, recovery_verified=True, no_live_execution=True)
