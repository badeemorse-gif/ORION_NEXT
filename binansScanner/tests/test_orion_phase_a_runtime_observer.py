...
class TestPhaseARuntimeObserver(unittest.TestCase):
    ...
    def test_observation_preserves_orion_outputs(self):
        result = self._result(decision="FAVORABLE", score=31.5)
    ...
    def test_d6_journal_compatibility_and_no_outcome(self):
        result = self._result()
        observation = extract_observation(result, "1h", timestamp=datetime(2026, 8, 17, 20, 0, tzinfo=timezone.utc)).observation
        assert observation is not None
    ...
    def test_wait_extracts_one_signal_time_observation(self):
        ...
        extraction = extract_wait_observation(result, "1h", timestamp=datetime(2026, 8, 17, 20, 0, tzinfo=timezone.utc))
        self.assertEqual(extraction.status, "OBSERVED")
        self.assertIsNotNone(extraction.observation)
        observation = extraction.observation
        assert observation is not None
    ...
    def test_point_in_time_and_future_leakage(self):
        observer_path = ROOT / "tools" / "orion_phase_a_runtime_observer.py"
        ...
        self.assertNotIn("OpportunityRelativeRanker", text)
        self.assertNotIn("OpportunityRankingInput", text)
        self.assertNotIn("SignalOutcome", text)
        for forbidden in ("forward_outcome_validation", "outcome_1h", "outcome_4h", "outcome_24h", "mfe", "mae"):
    ...
    def test_symbol_level_decision_creates_at_most_one_observation(self):
        ...
        self.assertIsNone(btc_entries[0].observation.directional_raw_strength)
        self.assertIsNone(btc_entries[0].observation.context_score)
        self.assertIsNone(btc_entries[0].observation.composite)
    ...
    def test_favorable_creates_one_observation_without_d3(self):
        class Stub:
            ...
            def run(self, symbol, timeframes):
                self.result = TestPhaseARuntimeObserver._result(symbol=symbol, decision="FAVORABLE")
                return self.result
        ...
        self.assertEqual(len(btc_entries), 1)
        self.assertEqual(btc_entries[0].observation.timeframe, "1h")
        self.assertEqual(btc_entries[0].observation.decision, "FAVORABLE")
        self.assertIsNone(btc_entries[0].observation.directional_raw_strength)
        self.assertIsNone(btc_entries[0].observation.context_score)
        self.assertIsNone(btc_entries[0].observation.composite)
        self.assertIsNone(btc_entries[0].observation.relative_rank)
        self.assertIsNone(btc_entries[0].observation.relative_percentile)
    ...
    def test_unfavorable_creates_one_observation_without_d3(self):
        class Stub:
            ...
            def run(self, symbol, timeframes):
                self.result = TestPhaseARuntimeObserver._result(symbol=symbol, decision="UNFAVORABLE", score=-31.5)
                return self.result
        ...
        self.assertEqual(len(btc_entries), 1)
        self.assertEqual(btc_entries[0].observation.timeframe, "1h")
        self.assertEqual(btc_entries[0].observation.decision, "UNFAVORABLE")
        self.assertIsNone(btc_entries[0].observation.directional_raw_strength)
        self.assertIsNone(btc_entries[0].observation.context_score)
        self.assertIsNone(btc_entries[0].observation.composite)
        self.assertIsNone(btc_entries[0].observation.relative_rank)
    ...
    def test_profile_blocked_creates_no_successful_observation(self):
        ...
        observer = PhaseARuntimeObserver(self._config(), orchestrator_factory=Stub)
        run = observer.run(REQUIRED_SYMBOLS, REQUIRED_TIMEFRAMES)
        ada_observed = [x for x in run.records if x["status"] == "OBSERVED" and x["symbol"] == "ADAUSDT"]
        self.assertEqual(len(ada_observed), 0)
    ...
    def test_no_execution_access_or_requests(self):
        observer_path = ROOT / "tools" / "orion_phase_a_runtime_observer.py"
        text = observer_path.read_text(encoding="utf-8")
        self.assertNotIn("ExecutionEngine", text)
        self.assertNotIn("PaperExecutionAdapter", text)
        self.assertNotIn("LiveExecutionAdapter", text)
        self.assertNotIn("order_endpoint", text)
