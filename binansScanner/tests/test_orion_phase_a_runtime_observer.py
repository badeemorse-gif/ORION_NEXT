...
from tools.orion_phase_a_runtime_observer import (
    REQUIRED_SYMBOLS, REQUIRED_TIMEFRAMES, UNAVAILABLE_AT_BOUNDARY,
    PhaseARuntimeObserver, _primary_timeframe, create_runtime_config,
    extract_observation, extract_wait_observation,
)
...
class TestPhaseARuntimeObserver(unittest.TestCase):
    ...
    def _result(symbol="BTCUSDT", decision="FAVORABLE", score=31.5):
        ...
        profile = ProfileResult(
            ...
            is_tradeable=True, generated_at=now,
        )
        return OrchestratorResult(
            dataset=dataset, analysis=AnalysisResult("BULLISH", 20.0, ["TEST_SIGNAL"]), profile=profile,
    ...
    def _config(self, runtime_commit="5db73bfb079655fd32e2127289f181938006a167"):
        return create_runtime_config(
            ...
            session_id="EXP-20260817T200000Z-abcdef123456", runtime_commit=runtime_commit,
        )
    ...
    def test_full_universe_accepted(self):
        config = self._config()
        self.assertTrue(config.universe_identity.startswith("UNIV-"))

    def test_valid_subset_btc_accepted(self):
        config = create_runtime_config(
            baseline_commit="c54dc67792776da905a3efb1f667c1869c15db3d", symbols=("BTCUSDT",),
            timeframes=REQUIRED_TIMEFRAMES, configuration={"execution": {"paper": False, "live": False}},
        )
        self.assertTrue(config.universe_identity.startswith("UNIV-"))

    def test_valid_subset_ada_accepted(self):
        config = create_runtime_config(
            baseline_commit="c54dc67792776da905a3efb1f667c1869c15db3d", symbols=("ADAUSDT",),
            timeframes=REQUIRED_TIMEFRAMES, configuration={"execution": {"paper": False, "live": False}},
        )
        self.assertTrue(config.universe_identity.startswith("UNIV-"))
    ...
    def test_unknown_symbol_rejected(self):
        with self.assertRaises(ValueError):
            create_runtime_config(
                baseline_commit="c54dc67792776da905a3efb1f667c1869c15db3d", symbols=("DOGEUSDT",),
                timeframes=REQUIRED_TIMEFRAMES, configuration={"execution": {"paper": False, "live": False}},
            )
    ...
    def test_duplicate_symbol_rejected(self):
        with self.assertRaises(ValueError):
            create_runtime_config(
                baseline_commit="c54dc67792776da905a3efb1f667c1869c15db3d", symbols=("BTCUSDT", "BTCUSDT"),
                timeframes=REQUIRED_TIMEFRAMES, configuration={"execution": {"paper": False, "live": False}},
            )
    ...
    def test_observation_preserves_orion_outputs(self):
        ...
        observation = extraction.observation; assert observation is not None
        self.assertEqual(observation.raw_score, 31.5); self.assertEqual(observation.decision, "FAVORABLE")
        self.assertEqual(observation.market_regime, "Markup"); self.assertEqual(observation.timeframe, "1h")
        self.assertIsNone(observation.directional_raw_strength); self.assertIsNone(observation.context_score)
        self.assertIsNone(observation.composite); self.assertIsNone(observation.relative_rank); self.assertIsNone(observation.relative_percentile)
    ...
    def test_d6_journal_compatibility_and_no_outcome(self):
        ...
        self.assertEqual(len(journal), 1); self.assertIsNone(journal.entries[0].outcome)

    def test_missing_boundary_is_not_guessed(self):
        result = self._result(); profile = replace(result.profile, market=replace(result.profile.market, market_phase="")); result = replace(result, profile=profile)
        extraction = extract_observation(result, "1h", timestamp=datetime(2026, 8, 17, 20, 0, tzinfo=timezone.utc))
        self.assertEqual(extraction.status, UNAVAILABLE_AT_BOUNDARY); self.assertIn("market_regime", extraction.unavailable_fields); self.assertIsNone(extraction.observation)
    ...
    def test_wait_extracts_one_signal_time_observation(self):
        ...
        self.assertEqual(extraction.status, "OBSERVED"); observation = extraction.observation; assert observation is not None
        self.assertEqual(observation.timeframe, "1h"); self.assertEqual(observation.decision, "WAIT")
        self.assertIsNone(observation.directional_raw_strength); self.assertIsNone(observation.context_score); self.assertIsNone(observation.composite)
        self.assertIsNone(observation.relative_rank); self.assertIsNone(observation.relative_percentile)

    def test_point_in_time_and_future_leakage(self):
        text = (ROOT / "tools" / "orion_phase_a_runtime_observer.py").read_text(encoding="utf-8"); tree = ast.parse(text); imports = []
        for node in ast.walk(tree):
            if isinstance(node, ast.Import): imports.extend(alias.name for alias in node.names)
            elif isinstance(node, ast.ImportFrom): imports.append(node.module or "")
        self.assertNotIn("engines.execution_engine", imports); self.assertNotIn("models.execution", imports)
        for forbidden in ("Opportunity", "OpportunityRelativeRanker", "OpportunityRankingInput", "build_ranking_inputs", "ranker.rank", "SignalOutcome", "forward_outcome_validation", "outcome_1h", "outcome_4h", "outcome_24h", "mfe", "mae"):
            self.assertNotIn(forbidden, text)

    def test_ast_import_guard_blocks_d3_imports(self):
        tree = ast.parse((ROOT / "tools" / "orion_phase_a_runtime_observer.py").read_text(encoding="utf-8")); forbidden_modules = {"engines.opportunity_relative_ranking", "models.opportunity"}
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names: self.assertNotIn(alias.name, forbidden_modules)
            elif isinstance(node, ast.ImportFrom): self.assertNotIn(node.module, forbidden_modules)
    ...
    def _single_result_orchestrator(self, decision):
        ...
        class Stub:
            def __init__(self):
                self.result = result; self._analysis_engine = mock.Mock(); self._analysis_engine._select_primary_timeframe.side_effect = lambda dataset: (None, Timeframe.H1)
            def run(self, symbol, timeframes):
                self.result = TestPhaseARuntimeObserver._result(symbol=symbol, decision=decision, score=31.5 if decision == "FAVORABLE" else -31.5); return self.result
            def last_result(self): return self.result
        return Stub()

    def test_directional_runtime_never_imports_d3_or_opportunity(self):
        real_import = builtins.__import__; requested = []
        def spy_import(name, globals=None, locals=None, fromlist=(), level=0):
            if name in {"engines.opportunity_relative_ranking", "models.opportunity"}:
                requested.append(name); raise AssertionError(f"forbidden runtime import: {name}")
            return real_import(name, globals, locals, fromlist, level)
        for decision in ("FAVORABLE", "UNFAVORABLE"):
            with mock.patch("builtins.__import__", side_effect=spy_import):
                run = PhaseARuntimeObserver(self._config(), orchestrator_factory=lambda d=decision: self._single_result_orchestrator(d)).run(REQUIRED_SYMBOLS, REQUIRED_TIMEFRAMES)
                btc = [e for e in run.journal.entries if e.observation.symbol == "BTCUSDT"]
                self.assertEqual(len(btc), 1); self.assertEqual(btc[0].observation.timeframe, "1h")
    ...
    def test_directional_matrix_and_d3_none_contract(self):
        for decision in ("FAVORABLE", "UNFAVORABLE"):
            ...
            self.assertEqual(len(btc_entries), 1); observation = btc_entries[0].observation; self.assertEqual(observation.timeframe, "1h"); self.assertEqual(observation.decision, decision)
            for field in ("directional_raw_strength", "context_score", "composite", "relative_rank", "relative_percentile"): self.assertIsNone(getattr(observation, field))
    ...
    def test_wait_matrix(self):
        ...
        btc_entries = [entry for entry in run.journal.entries if entry.observation.symbol == "BTCUSDT"]
        self.assertEqual(len(btc_entries), 1); self.assertEqual(btc_entries[0].observation.decision, "WAIT"); self.assertEqual(btc_entries[0].observation.timeframe, "1h")
    ...
    def test_primary_selector_identity(self):
        selector_calls = []; created = []; result = self._result(decision="WAIT")
        class SelectorSpy:
            def __init__(self, owner): self.owner = owner; self.calls = 0
            def __call__(self, dataset): self.calls += 1; selector_calls.append(self.owner); return None, Timeframe.H1
        class Stub:
            def __init__(self):
                self.result = result; self._analysis_engine = mock.Mock(); self._analysis_engine._select_primary_timeframe = SelectorSpy(self); created.append(self)
        ...
        self.assertTrue(run.journal.entries); self.assertEqual(len(selector_calls), len(REQUIRED_SYMBOLS) * 2)
        self.assertEqual({id(owner) for owner in selector_calls}, {id(instance) for instance in created})
        for instance in created: self.assertGreater(instance._analysis_engine._select_primary_timeframe.calls, 0)
    ...
    def test_missing_primary_selector_fails_closed_without_new_analysis_engine(self):
        ...
        class ForbiddenAnalysisEngine:
            def __init__(self, *args, **kwargs): constructed.append(True); raise AssertionError("fallback AnalysisEngine construction is forbidden")
        result = self._result(decision="WAIT")
        class MissingSelectorEngine: pass
        class Stub:
            def __init__(self): self.result = result; self._analysis_engine = MissingSelectorEngine()
            def run(self, symbol, timeframes): self.result = TestPhaseARuntimeObserver._result(symbol=symbol, decision="WAIT"); return self.result
            def last_result(self): return self.result
        with mock.patch("tools.orion_phase_a_runtime_observer.AnalysisEngine", ForbiddenAnalysisEngine):
            ...
            self.assertIsNone(_primary_timeframe(Stub(), result)); run = observer.run(REQUIRED_SYMBOLS, REQUIRED_TIMEFRAMES)
        self.assertEqual(constructed, []); btc = [record for record in run.records if record["symbol"] == "BTCUSDT"]
    ...
    def test_profile_blocked_creates_no_successful_observation(self):
        class Stub:
            ...
            def run(self, symbol, timeframes):
                result = TestPhaseARuntimeObserver._result(symbol=symbol); result.statistics.current_stage = PipelineStage.PROFILE; result.statistics.success = False; result.statistics.error_message = "Profile intelligence blocked before Score/Decision: Extreme market risk"; result.score = None; result.decision = None; self.result = result; raise PipelineError(result.statistics.error_message)
            def last_result(self): return self.result
        run = PhaseARuntimeObserver(self._config(), orchestrator_factory=Stub).run(REQUIRED_SYMBOLS, REQUIRED_TIMEFRAMES)
        ada_records = [record for record in run.records if record["symbol"] == "ADAUSDT"]
        self.assertEqual(len([record for record in ada_records if record["status"] == "OBSERVED"]), 0); self.assertIn("PIPELINE_BLOCKED", {record["status"] for record in ada_records})

    def test_no_score_or_decision_recalculation_in_observer(self):
        class Stub:
            def __init__(self): self.result = None; self._analysis_engine = mock.Mock(); self._analysis_engine._select_primary_timeframe.side_effect = lambda dataset: (None, Timeframe.H1)
        ...
        self.assertEqual(obs["source_outputs"]["score"]["raw_score"], 31.5); self.assertEqual(obs["source_outputs"]["decision"]["decision"], "FAVORABLE")

    def test_execution_isolation(self):
        tree = ast.parse((ROOT / "tools" / "orion_phase_a_runtime_observer.py").read_text(encoding="utf-8")); forbidden = {"engines.execution_engine", "adapters.paper_execution", "adapters.live_execution", "models.execution"}
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names: self.assertNotIn(alias.name, forbidden)
            elif isinstance(node, ast.ImportFrom): self.assertNotIn(node.module, forbidden)
