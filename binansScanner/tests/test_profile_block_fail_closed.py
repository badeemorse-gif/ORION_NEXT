...
class TestProfileBlockFailClosed(TestCase):
    ...
    def _profile(*, tradeable: bool) -> ProfileResult:
        return ProfileResult(
            ...
            timeframes=(),
            warnings=(),
            blocks=() if tradeable else ("PROFILE_BLOCK",),
    ...
    def _orchestrator(self, profile_result: ProfileResult):
        ...
        score.calculate.return_value = ScoreResult(
            score=0.0,
            category="NEUTRAL",
            factors=["PROFILE_BLOCK_FIXTURE"],
        )
        decision = MagicMock()
        decision.decide.return_value = DecisionResult(
            decision="WAIT",
            confidence=0.0,
            reasons=["PROFILE_BLOCK_FIXTURE"],
        )
        plan_builder = MagicMock()
        plan_builder.build.return_value = MagicMock(name="execution_plan")
        ...
        orchestrator = Orchestrator(
            ...
            execution_plan_builder=plan_builder,
        )
    ...
    def test_blocked_profile_fails_closed_before_score_decision_and_plan(self) -> None:
        orchestrator, score, decision, plan_builder = self._orchestrator(
            self._profile(tradeable=False)
        )

        with self.assertRaisesRegex(PipelineError, "Profile intelligence blocked before Score/Decision"):
            orchestrator.run_pipeline("BTCUSDT", ["1m"])

        stats = orchestrator.statistics()
        self.assertIsNotNone(stats)
        self.assertEqual(stats.current_stage, PipelineStage.PROFILE)
        self.assertFalse(stats.success)
        self.assertIsNone(orchestrator.last_result().execution_plan)
        score.calculate.assert_not_called()
        decision.decide.assert_not_called()
        plan_builder.build.assert_not_called()

    def test_tradeable_profile_preserves_existing_score_decision_and_plan_path(self) -> None:
        orchestrator, score, decision, plan_builder = self._orchestrator(
            self._profile(tradeable=True)
        )

        result = orchestrator.run_pipeline("BTCUSDT", ["1m"])

        score.calculate.assert_called_once()
        decision.decide.assert_called_once_with(
            orchestrator._last_result.analysis,
            score.calculate.return_value,
        )
        plan_builder.build.assert_called_once_with(
            result.dataset,
        ...
        self.assertIs(result.execution_plan, plan_builder.build.return_value)
