...
class ShortHistoryNetwork:
    def __init__(self, persistent_short: bool = False):
        self.persistent_short = persistent_short
    ...
    def __call__(self, request, timeout):
        ...
        if "exchangeInfo" in url:
            return Response(exchange_info_rows())
        if "ticker/24hr" in url:
            if "symbol=" in url:
                self.targeted_calls.append(url)
                return Response(ticker(DJTB))
...
class D2HistoryRecoveryTests(unittest.TestCase):
    def test_provider_short_history_retries_without_caching_invalid_payload(self):
        ...
        with patch("providers.binance_opportunity_source.urlopen", side_effect=network):
            ...
            history = source._fetch_history(DJTB)

        self.assertEqual(len(history), 32)
        self.assertEqual(network.history_calls, [DJTB, DJTB])
        self.assertEqual(source._cache[f"klines_{DJTB}"].value, history)
    ...
    def test_provider_persistent_short_history_exhausts_bounded_recovery(self):
        ...
        with patch("providers.binance_opportunity_source.urlopen", side_effect=network):
            source = BinanceSpotOpportunitySource(ttl_seconds=300.0)
            with self.assertRaisesRegex(ValueError, "insufficient price history"):
                source._fetch_history(DJTB)

        self.assertEqual(network.history_calls, [DJTB, DJTB])
        self.assertNotIn(f"klines_{DJTB}", source._cache)
        recovery = [event for event in source.reconciliation_events if event["symbol"] == DJTB]
        self.assertEqual(recovery[-1]["history_outcome"], "exhausted")
        self.assertEqual(recovery[-1]["final_disposition"], "unresolved")
    ...
    def test_provider_malformed_history_fails_without_recovery_loop(self):
        ...
        with patch("providers.binance_opportunity_source.urlopen", side_effect=network):
            source = BinanceSpotOpportunitySource(ttl_seconds=300.0)
            with self.assertRaisesRegex(ValueError, "invalid price history candle"):
                source._fetch_history(DJTB)

        self.assertEqual(network.history_calls, [DJTB])
        self.assertNotIn(f"klines_{DJTB}", source._cache)
    ...
    def test_real_paper8h_create_recovers_short_history_before_metric_construction(self):
        ...
        network = ShortHistoryNetwork()
        lifecycle = Mock()
        original_builder = OpportunityDiscovery._build_history_metric
        built_lengths: list[int] = []

        def traced_builder(self, source, symbol, ticker_value, book_value, history):
            built_lengths.append(len(history))
            return original_builder(self, source, symbol, ticker_value, book_value, history)

        with tempfile.TemporaryDirectory() as tmp:
            config = runner_module.Paper8HConfig(
                duration_hours=0.01,
            ...
            ) as lifecycle_factory, patch.object(
                OpportunityDiscovery, "_build_history_metric", new=traced_builder
            ):
        ...
        self.assertIn(32, built_lengths)
        self.assertNotIn(3, built_lengths)
