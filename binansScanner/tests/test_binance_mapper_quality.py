...
class TestBinanceMapperQuality(unittest.TestCase):
    ...
    def test_mapper_returns_only_quality_valid_dataset(self):
        dataset = self.mapper.create_market_dataset(symbol="BTCUSDT", timeframe_data={"1h": self._frame()})
        self.assertTrue(dataset.is_valid)
        self.assertEqual(len(dataset.timeframes), 1)

    def test_mapper_rejects_timeframe_gap(self):
        with self.assertRaises(InvalidKlinesData):
            self.mapper.create_market_dataset(symbol="BTCUSDT", timeframe_data={"1h": self._frame(gap=True)})

    def test_mapper_rejects_non_finite_values(self):
        frame = self._frame()
        frame.loc[frame.index[0], "close"] = np.inf
        with self.assertRaises(InvalidKlinesData):
            self.mapper.create_market_dataset(symbol="BTCUSDT", timeframe_data={"1h": frame})
