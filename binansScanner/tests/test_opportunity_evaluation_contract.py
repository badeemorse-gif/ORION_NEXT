...
from models.opportunity_evaluation import (
...
class TestOpportunityEvaluationContract(unittest.TestCase):
    def _eligible(self) -> Opportunity:
        return Opportunity(
            ...
            confidence=82.0,
            setup_quality=78.0,
            risk=OpportunityRisk(state=RiskState.ACCEPTABLE, invalidation="setup invalidation"),
            expected_move=1.5,
            supporting_evidence=("trend alignment",),
    ...
    def test_eligible_opportunity_can_be_accepted(self) -> None:
        evaluation = OpportunityEvaluation(
            opportunity=self._eligible(),
            status=OpportunityEvaluationStatus.ACCEPTED,
        )
    ...
    def test_rejected_evaluation_preserves_reason(self) -> None:
        evaluation = OpportunityEvaluation(
            opportunity=self._eligible(),
            status=OpportunityEvaluationStatus.REJECTED,
            reasons=("candidate excluded by future ranking policy",),
        )
    ...
    def test_naive_evaluation_timestamp_is_rejected(self) -> None:
        with self.assertRaises(ValueError):
            OpportunityEvaluation(
                opportunity=self._eligible(),
                status=OpportunityEvaluationStatus.ACCEPTED,
                evaluated_at=datetime(2026, 8, 11, 12, 0),
            )
