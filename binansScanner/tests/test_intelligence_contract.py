...
class TestIntelligenceContract(unittest.TestCase):
    def _profile(self, *, warnings=()):
        ...
        c = MarketCharacteristics(trend="Bullish", trend_strength="Strong", momentum="Buy", volume_strength="Strong", volatility_level="Normal", ema_alignment="Bullish", market_phase="Markup", risk_level="Medium", confidence=80.0, trend_score=80.0, momentum_score=80.0, volume_score=80.0, volatility_score=40.0)
        s = ProfileStatistics(health_score=80.0, confidence_limit=80.0, completion_ratio=1.0, total_candles=100, missing_candles=0)
        t = TimeframeProfile(timeframe="1h", characteristics=c, candles_count=100, first_timestamp=now, last_timestamp=now, missing_candles=0)
        return ProfileResult(symbol="BTCUSDT", market=c, statistics=s, timeframes=(t,), warnings=warnings, is_tradeable=True)
    ...
    def test_warning_bearing_profile_is_not_actionable(self):
        with self.assertRaises(IntelligenceContractError): validate_profile(self._profile(warnings=("PROFILE_WARNING",)))
    ...
    def test_wait_requires_zero_confidence(self):
