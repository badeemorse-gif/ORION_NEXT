...
from models.signal_snapshot import MaterialChangePolicy, SignalIdentity, SignalSnapshot
...
class TestPaperRealtimeLifecycleIntegration(unittest.TestCase):
    def test_buy_100_reprice_118_old_order_never_fills(self) -> None:
        ...
        action = runtime.revalidate(
            intent_id=first.intent_id,
            snapshot=snapshot(version=2, price=118.0, generated_at=t1),
            market_price=120.0,
            now=t1,
        )
        ...
        self.assertEqual(len(runtime.ledger.events), 3)
    ...
    def test_wait_cancels_pending(self) -> None:
        ...
        action = runtime.revalidate(
            intent_id=first.intent_id,
            snapshot=snapshot(version=2, price=100.0, decision="WAIT", generated_at=t0 + timedelta(minutes=1)),
            market_price=105.0,
    ...
    def test_sell_signal_cancels_pending_buy(self) -> None:
        ...
        action = runtime.revalidate(
            intent_id=first.intent_id,
            snapshot=snapshot(version=2, price=99.0, direction="SELL", generated_at=t0 + timedelta(minutes=1)),
            market_price=101.0,
            now=t0 + timedelta(minutes=1),
        )
        self.assertEqual(action.value, "CANCEL")
    ...
    def test_repricing_limit_returns_no_trade(self) -> None:
        ...
        action = runtime.revalidate(
            intent_id=first.intent_id,
            snapshot=snapshot(version=2, price=118.0, generated_at=t0 + timedelta(minutes=1)),
            market_price=120.0,
            now=t0 + timedelta(minutes=1),
        )
    ...
    def test_duplicate_market_event_is_suppressed(self) -> None:
        ...
        self.assertEqual(len(runtime.ledger.events), 2)
    ...
    def test_exit_updates_capital_and_replay_is_stable(self) -> None:
        ...
        self.assertEqual(state_a.positions[0].quantity, 0.0)
        self.assertAlmostEqual(state_a.cash, 210.0)
