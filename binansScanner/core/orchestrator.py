...
class Orchestrator:
    def __init__(self, provider: MarketProviderProtocol, storage: MarketStorageProtocol, indicator_engine: IndicatorEngine, analysis_engine: AnalysisEngine, profile_engine: ProfileEngine, score_engine: ScoreEngine, decision_engine: DecisionEngine, validation_engine: ValidationEngine, config: OrchestratorConfig, execution_plan_builder: Optional[ExecutionPlanBuilder] = None) -> None:
        dependencies = {
            "provider": provider, "storage": storage, "indicator_engine": indicator_engine,
            "analysis_engine": analysis_engine, "profile_engine": profile_engine,
            "score_engine": score_engine, "decision_engine": decision_engine,
            "validation_engine": validation_engine, "config": config,
        }
    ...
    def run_pipeline(self, symbol: str, timeframes: list[str]) -> OrchestratorResult:
        ...
        try:
            ...
            self._last_result = OrchestratorResult(
                dataset=dataset, validation=validation, analysis=analysis,
                profile=profile, score=score, decision=decision,
                execution_plan=execution_plan,
                statistics=PipelineStatistics(started_at=started, finished_at=finished, elapsed_ms=elapsed, current_stage=self._current_stage, completed_stage_count=completed, success=success, error_message=error_message),
            )
