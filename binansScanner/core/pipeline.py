...
class Pipeline:
    ...
    def __init__(self, orchestrator: Orchestrator, execution_engine: ExecutionEngine, report_engine: ReportEngine, logger: Optional[logging.Logger] = None) -> None:
    ...
    def run_symbol(self, symbol: str, timeframes: list[str], quantity: Optional[float] = None) -> PipelineItemResult:
        ...
        try:
            ...
            report_res = self._report_engine.build_report(
                ...
                execution=exec_res,
                execution_time_ms=orch_res.statistics.elapsed_ms + exec_res.execution_time_ms,
                stage_trace=("ORCHESTRATION", "EXECUTION", "REPORT"),
                failure_stage="EXECUTION" if execution_failed else None,
                failure_message=(exec_res.message or "ExecutionEngine returned FAILED.") if execution_failed else None,
            )
            if not isinstance(report_res, ReportResult):
                raise PipelineError("ReportEngine returned an invalid ReportResult.")
            if execution_failed:
                failed_stage = "EXECUTION"
                raise PipelineError(f"Execution failed: {exec_res.message or 'ExecutionEngine returned FAILED.'}")
            success = True
            failed_stage = None
            ...
            if report_res is None and orch_res is not None:
                try:
                    trace = (("ORCHESTRATION", "EXECUTION", "REPORT") if exec_res is not None else ("ORCHESTRATION", "REPORT"))
                    report_res = self._report_engine.build_report(
                        symbol=symbol,
                        analysis=orch_res.analysis,
                        profile=orch_res.profile,
                        ...
                        decision=orch_res.decision,
                        execution=exec_res,
                        execution_time_ms=orch_res.statistics.elapsed_ms + (exec_res.execution_time_ms if exec_res is not None else 0.0),
                        stage_trace=trace,
                    ...
                    self._logger.error(f"Failure evidence report could not be built for [{symbol}]: {report_exc}")
            self._logger.error(f"Pipeline failed for [{symbol}] at stage [{failed_stage}]: {exc}")
        finished = datetime.now(timezone.utc)
        elapsed = (time.perf_counter() - perf) * 1000.0
        return PipelineItemResult(symbol=symbol, orchestrator_result=orch_res, execution_result=exec_res, report_result=report_res, started_at=started, finished_at=finished, elapsed_ms=elapsed, success=success, error_message=error_message, failed_stage=failed_stage)
    ...
    def run_symbols(self, symbols: Iterable[str], timeframes: list[str], quantity: Optional[float] = None) -> tuple[PipelineSummary, list[PipelineItemResult]]:
        started = datetime.now(timezone.utc)
        perf = time.perf_counter()
        results = [self.run_symbol(symbol, timeframes, quantity) for symbol in list(symbols)]
        processed = len(results)
        successful = sum(1 for item in results if item.success)
        failed = processed - successful
        executed = sum(1 for item in results if item.execution_result is not None and item.execution_result.status is ExecutionStatus.EXECUTED)
        finished = datetime.now(timezone.utc)
        elapsed = (time.perf_counter() - perf) * 1000.0
        summary = PipelineSummary(started_at=started, finished_at=finished, elapsed_ms=elapsed, processed_symbols=processed, successful_symbols=successful, failed_symbols=failed, execution_count=executed, success=processed > 0 and failed == 0)
