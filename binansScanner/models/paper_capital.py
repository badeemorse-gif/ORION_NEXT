...
class FeeModel:
    ...
    def __post_init__(self) -> None:
        if self.rate < 0.0 or self.minimum < 0.0:
...
class LedgerEvent:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "timestamp", _utc(self.timestamp, "timestamp"))
        for name in ("quantity", "price", "notional", "fee", "slippage", "realized_pnl", "cash_delta", "position_quantity_delta"):
...
class PaperLedger:
    ...
    def snapshot(self, timestamp: datetime) -> "PaperLedger":
        state = self.replay()
        return self.append(self._event(LedgerEventType.SNAPSHOT, timestamp, cash_delta=0.0, notional=state.equity))
    ...
    def replay(self) -> PaperAccountState:
        ...
        return PaperAccountState(wallet, tuple(sorted(positions.values(), key=lambda p: p.symbol)), tuple(sorted(prices.items())), realized, peak, max_dd)

    def latest_equity_snapshot(self) -> EquitySnapshot:
        state = self.replay()
        return EquitySnapshot(
            timestamp=self.events[-1].timestamp if self.events else datetime.now(timezone.utc),
            equity=state.equity,
            cash=state.wallet.cash,
            reserved_cash=state.wallet.reserved_cash,
            available_cash=state.wallet.available_cash,
