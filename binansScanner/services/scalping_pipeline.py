...
class FastRecall:
    ...
    def recall(self, candidates: Sequence[OpportunityCandidate], *, broad_limit: int) -> RecallResult:
        ...
        # The broad cap is a pool size, not a discovery filter. Each recall lane is
        # intentionally small so all lanes can contribute before composite ordering.
...
class ScalpingOpportunityPipeline:
    ...
    @staticmethod
    def _rebuild(candidate: OpportunityCandidate, *, opportunity_class: str | None = None, entry_state: str | None = None,
                 entry_allowed: bool | None = None, extra_rejections: tuple[RejectionReason, ...] = (),
                 extra_reasons: tuple[str, ...] = ()) -> OpportunityCandidate:
    ...
    def _classification_integrity(self, candidate: OpportunityCandidate) -> OpportunityCandidate:
        ...
        # C/D are never actionable, irrespective of what an upstream score says.
    ...
    def discover(self, top_n: int | None = None, **decision_kwargs) -> ScalpingCandidateSet:
        ...
        # A very large discovery limit requests the eligible universe without making
        # active_top_n an implicit discovery boundary. No daily-candle gate occurs here.
