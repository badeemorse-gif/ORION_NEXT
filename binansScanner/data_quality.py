...
class MarketDatasetQualityValidator:
    ...
    def validate(self, dataset: MarketDataset, *, required_timeframes: Iterable[Timeframe] = (), now: Optional[datetime] = None, max_age: Optional[timedelta] = None) -> DataQualityReport:
        ...
        issues: list[str] = []
        missing_required = [timeframe for timeframe in required_timeframes if not dataset.has_timeframe(timeframe)]
        if missing_required:
            issues.extend(f"missing required timeframe: {timeframe.value}" for timeframe in missing_required)
        ...
        for timeframe, timeframe_data in dataset.timeframes.items():
            ...
            if max_age is not None and timeframe_data.last_timestamp is not None:
                try:
                    age = checked_at - self._ensure_aware(timeframe_data.last_timestamp, "last_timestamp")
                    if age > max_age:
                        issues.append(f"stale timeframe: {timeframe.value} (age={age}, max_age={max_age})")
                except DataQualityError as exc:
                    issues.append(str(exc))
        if missing_required:
            status = DataQualityStatus.MISSING
        elif issues and not all(issue.startswith("stale timeframe:") for issue in issues):
            status = DataQualityStatus.INVALID
        elif issues:
            status = DataQualityStatus.STALE
        else:
            status = DataQualityStatus.VALID
        return DataQualityReport(status=status, checked_at=checked_at, issues=tuple(issues))

    def assert_valid(self, dataset: MarketDataset, *, required_timeframes: Iterable[Timeframe] = (), now: Optional[datetime] = None, max_age: Optional[timedelta] = None) -> DataQualityReport:
        report = self.validate(dataset, required_timeframes=required_timeframes, now=now, max_age=max_age)
        if not report.is_valid:
            details = "; ".join(report.issues) or report.status.value
            raise DataQualityError(f"MarketDataset failed quality contract: {details}")
        return report
    ...
    def _validate_metadata(self, dataset: MarketDataset) -> list[str]:
        ...
        try:
            ...
            if last_updated_at < downloaded_at:
                issues.append("invalid provenance chronology: last_updated_at before downloaded_at")
        except DataQualityError as exc:
            issues.append(str(exc))
    ...
    def _validate_timeframe(self, timeframe: Timeframe, timeframe_data) -> list[str]:
        ...
        if timeframe_data.timeframe is not timeframe:
            issues.append(f"timeframe key mismatch: key={timeframe.value}, payload={timeframe_data.timeframe.value}")
        if not isinstance(dataframe, pd.DataFrame) or dataframe.empty:
            issues.append(f"invalid or empty dataframe: {timeframe.value}")
            return issues
        missing_columns = [column for column in self.REQUIRED_OHLCV_COLUMNS if column not in dataframe.columns]
        if missing_columns:
            issues.append(f"missing OHLCV columns for {timeframe.value}: {missing_columns}")
            return issues
        ...
        non_numeric_columns = [column for column in self.REQUIRED_OHLCV_COLUMNS if not pd.api.types.is_numeric_dtype(required_data[column])]
        if non_numeric_columns:
            issues.append(f"non-numeric OHLCV columns for {timeframe.value}: {non_numeric_columns}")
            return issues
        try:
        ...
        invalid_ohlc = ((dataframe["high"] < dataframe["low"]) | (dataframe["high"] < dataframe["open"]) | (dataframe["high"] < dataframe["close"]) | (dataframe["low"] > dataframe["open"]) | (dataframe["low"] > dataframe["close"]))
        if invalid_ohlc.any():
        ...
        if expected_interval is None:
            ...
            if not (deltas == expected_interval).all():
                issues.append(f"timeframe cadence/gap violation: {timeframe.value} expected {expected_interval}")
        if timeframe_data.candles_count != len(dataframe):
            issues.append(f"candles_count mismatch: {timeframe.value} metadata={timeframe_data.candles_count}, actual={len(dataframe)}")
        try:
