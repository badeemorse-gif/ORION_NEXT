...
class HtmlReportRenderer:
    ...
    def render(self, report: ReportResult) -> str:
        ...
        try:
            ...
            score = self._value(getattr(report.score, "total_score", None), getattr(report.score, "score", "N/A"))
            ...
            if report.warnings:
                ...
                sections.append("</div>")
            sections.extend(['<div class="card"><h2>Report Data</h2>', f"<pre>{html.escape(self._pretty_dump(report))}</pre>", "</div></div></body></html>"])
