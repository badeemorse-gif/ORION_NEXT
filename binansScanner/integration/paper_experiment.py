...
class PaperExperimentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.universe_mode == "dynamic" and self.fixed_allocation is not None and self.fixed_allocation < 0:
...
class PaperExperimentMetrics:
    """A/B report schema with deterministic metric definitions.

    Inputs are canonical observations, not fabricated placeholders. Rates are
    calculated from their explicitly named denominators; zero-denominator
    rates are represented as 0.0 because no opportunity existed to observe the
    event. Profit factor is 0.0 when there are no profitable closed trades.
    """
    ...
    def __post_init__(self) -> None:
        ...
        for name in (
            ...
            "profit_factor", "capital_utilization", "average_hold_time",
            "opportunity_capture_rate", "entry_acceptance_rate", "false_negative_rate",
        ):
...
def build_metrics(*, account: Mapping[str, float], observation: PaperExperimentObservation) -> PaperExperimentMetrics:
    ...
    profit_factor = gross_profit / gross_loss if gross_loss > 0.0 else (1.0 if gross_profit > 0.0 else 0.0)
    ...
    return PaperExperimentMetrics(
        ...
        false_negative_rate=_rate(observation.false_negatives, observation.actionable_outcomes),
