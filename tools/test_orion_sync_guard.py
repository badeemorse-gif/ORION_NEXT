...
class TestSyncGuardContract(unittest.TestCase):
    ...
    def test_audit_requires_all_public_launchers_to_use_guard(self):
        originals = {}
        for path in guard.MIRROR_ENTRYPOINTS:
            if path.exists():
                originals[path] = path.read_text(encoding="utf-8")
        try:
            for launcher in (
                guard.REPO_ROOT / "tools" / "orion_sync.bat",
                guard.REPO_ROOT / "tools" / "orion_main_sync.bat",
                guard.REPO_ROOT / "tools" / "orion_all_sync.bat",
            ):
                if launcher.exists():
                    launcher.write_text(launcher.read_text(encoding="utf-8").replace("orion_sync_guard.py", "legacy_sync.py"), encoding="utf-8")
