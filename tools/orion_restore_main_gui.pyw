"""ORION Restore combined UI: MAIN exact official mirror + frozen ALL engine.

ALL is loaded from the existing orion_restore_gui.pyw without changing its
implementation. MAIN is a second path that mirrors origin/main exactly into
PROJECT_ROOT while leaving the checkout's .git directory untouched.
"""
...
def main_manifest(root: str) -> Manifest:
    """Manifest of the project content, deliberately excluding the Git metadata directory."""
...
def _main_parent_directory(path: str, root: str) -> None:
    ...
    while parent != root_abs:
        ...
        if os.path.lexists(parent) and (os.path.islink(parent) or not os.path.isdir(parent)):
            raise RestoreError(f"MAIN destination parent collision: {parent}")
...
def materialize_main(archive: bytes, root: str, target: Manifest) -> BranchStats:
    """Materialize origin/main as an exact mirror, excluding only .git metadata."""
    root_abs = os.path.abspath(root)
    ...
    # MAIN is intentionally an exact mirror. Everything outside the Git target
    # snapshot is stale, including local files that are untracked by Git.
    stale = sorted((rel for rel in old if rel not in target_paths), key=lambda item: item.count("/"), reverse=True)
    removed = sum(1 for rel in stale if old[rel][0] in ("file", "link"))
    for rel in stale:
...
def verify_main_materialized(root: str, target: Manifest) -> None:
    """Verification is exact: every non-.git path must match the main snapshot."""
...
def sync_main(project_root: str, remote: str = REMOTE, report=None) -> BranchStats:
    ...
    stats = materialize_main(archive, project_root, target)
    # State is retained outside PROJECT_ROOT only for compatibility with older
    # releases; exact MAIN synchronization no longer relies on it.
    if report:
