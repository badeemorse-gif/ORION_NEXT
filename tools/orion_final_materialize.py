...
def fetch_exact_commit(remote: str, commit: str) -> str:
    """Fetch the requested object from origin and return its resolved commit SHA."""
...
def verify_snapshot(root: Path, expected: dict[str, tuple], label: str) -> None:
    ...
    if extra:
        details.append("extra=" + ",".join(extra[:12]))
    if different:
        details.append("different=" + ",".join(different[:12]))
    ...
    with tarfile.open(fileobj=io.BytesIO(data), mode="r:") as tar:
        for member in tar.getmembers():
            ...
            if target != destination.resolve() and destination.resolve() not in target.parents:
                fail(f"Archive extraction escaped staging directory: {rel}")
            if member.isdir():
                ...
                source = tar.extractfile(member)
                if source is None:
                    fail(f"Cannot extract archive member: {rel}")
                ...
                fail(f"Unsupported archive member type during extraction: {rel}")
    ...
    try:
        ...
        if target.exists() or target.is_symlink():
            if target.is_symlink() or not target.is_dir():
                fail(f"REFUSED: target is not a directory: {target}")
