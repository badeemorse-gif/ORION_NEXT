...
class OrionRestoreApp:
    ...
    def _build_ui(self):
        tk.Label(
            self.root, text="ORION RESTORE", font=("Segoe UI", 22, "bold"),
        ...
        tk.Label(
            branch_row, text="Branch:", width=12, anchor="w",
            font=("Segoe UI", 9, "bold"), bg=self.panel, fg=self.muted
        ).pack(side="left")
        self.branch_combo = ttk.Combobox(
            branch_row, textvariable=self.branch_var, values=BRANCHES,
        ...
        tk.Label(
            status, text="الحالة:", font=("Segoe UI", 10, "bold"),
            bg=self.bg, fg=self.muted
        ).pack(side="left")
        self.status_label = tk.Label(
            status, textvariable=self.status_var,
            font=("Segoe UI", 10, "bold"), bg=self.bg, fg=self.success
        )
        ...
        for i, (title, var, color) in enumerate((
            ("FILES", self.files_var, self.accent),
            ("ADDED", self.added_var, self.success),
            ("UPDATED", self.updated_var, self.accent),
            ("REMOVED", self.removed_var, self.error),
        )):
            stats.grid_columnconfigure(i, weight=1)
            card = tk.Frame(
                stats, bg="#182538", highlightthickness=1,
                highlightbackground="#24364b"
            )
            card.grid(
                row=0, column=i, sticky="nsew",
                padx=(0 if i == 0 else 4, 4 if i < 3 else 0)
            )
            tk.Label(
                card, text=title, font=("Segoe UI", 8, "bold"),
                bg="#182538", fg=self.muted
            ).pack(anchor="w", padx=10, pady=(7, 1))
            tk.Label(
                card, textvariable=var, font=("Segoe UI", 17, "bold"),
                bg="#182538", fg=color
            ).pack(anchor="w", padx=10, pady=(0, 7))

        self.button = tk.Button(
            self.root, text="⬇  استعادة مطابقة للمستودع",
            command=self.start_restore, font=("Segoe UI", 14, "bold"),
        ...
        tk.Label(
            self.root,
            text="⚠ استعادة Exact Mirror: سيتم حذف تعديلات Git والملفات/المجلدات غير المتتبعة والـignored غير الموجودة في الفرع الهدف.",
            font=("Segoe UI", 9), bg=self.bg, fg=self.warning,
            wraplength=820, justify="center"
        ).pack(pady=(0, 12))
        tk.Label(
            self.root, text="سجل العملية", font=("Segoe UI", 10, "bold"),
            bg=self.bg, fg=self.muted
        ...
        self._write("اختر main أو Phase 2 ثم نفّذ الاستعادة.")
        self._write("النتيجة المطلوبة: Local working tree == GitHub target revision.")

    def _branch_changed(self, _event=None):
        if not self.running:
            self.source_label.configure(text=f"GitHub → Git → Local  |  {self.branch}")
    ...
    def _info_row(self, parent, label, value):
        ...
        tk.Label(
            row, text=f"{label}:", width=12, anchor="w",
            font=("Segoe UI", 9, "bold"), bg=self.panel, fg=self.muted
        ).pack(side="left")
        tk.Label(
            row, text=value, anchor="w", font=("Segoe UI", 9),
    ...
    def _restore(self):
        ...
        try:
            ...
            if current_branch != branch:
                ...
                switch = ["switch", branch] if exists == 0 else [
                    "switch", "-c", branch, "--track", remote_ref
                ]
                code, lines = self._git(switch)
                if code != 0:
                    raise RuntimeError(
                        "تعذر التحويل إلى الفرع الهدف.\n" +
            ...
            if status_lines:
                raise RuntimeError(
                    "بقيت عناصر محلية بعد الاستعادة؛ لم نعلن التطابق.\n" +
    ...
    def _confirm(self, local_commit, remote_commit, extra_count, branch):
        ...
        def ask():
            ...
            result["ok"] = messagebox.askyesno(
                "تأكيد Exact Restore", message, icon="warning"
            )
            event.set()
