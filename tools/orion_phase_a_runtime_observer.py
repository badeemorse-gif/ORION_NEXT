 ...
 def run(self,symbols,timeframes):
  ...
  for symbol in symbols:
   ...
   try:r=o.run(symbol,list(timeframes)); status="SUCCESS"
    ...
    runtime.append({"symbol":symbol,"status":status,"stage":r.statistics.current_stage.value,"error":str(exc),"critical_indicators":_critical_indicator_status(r),"profile_warnings":list(r.profile.warnings) if r.profile else [],"profile_blocks":list(r.profile.blocks) if r.profile else [],"profile_status":"BLOCKED" if r.statistics.current_stage.value=="PROFILE" else ("PASS" if r.profile and r.profile.is_tradeable else "BLOCKED"),"score_status":"PRODUCED" if r.score else "NOT_PRODUCED","decision_status":"PRODUCED" if r.decision else "NOT_PRODUCED","execution":{"paper":False,"live":False}})
   else:
  ...
  for symbol,r in results:
   ...
   for tf in timeframes:
    if decision == "WAIT":
     ...
     records.append(self._record(ex,r,symbol,tf,ts));continue
