...
class PhaseARuntimeObserver:
 ...
 def run(self,symbols,timeframes):
  ...
  for symbol,r in results:
   ...
   for tf in timeframes:
    if direction is None:records.append(self._unavailable(symbol,tf,ts,("directional_raw_strength","context_score","composite","relative_rank","relative_percentile"),"Decision did not produce a directional signal."));continue
