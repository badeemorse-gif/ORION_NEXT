...
def _commit(x: str) -> str:
    x = x.strip().lower()
    if len(x) != 40 or any(c not in "0123456789abcdef" for c in x):
...
class _ClosedLookbackMarketSource:
    """Experimental acquisition adapter; production provider remains untouched."""

    def __init__(self, source: BinanceProvider, closed_candles: int = CLOSED_CANDLE_LOOKBACK):
        if closed_candles < 1:
    ...
    def download_timeframe(self, symbol: str, timeframe: Timeframe, limit: int = 1000):
...
def create_runtime_config(*, baseline_commit: str, symbols: Sequence[str], timeframes: Sequence[str], configuration: Mapping[str, Any], runtime_commit: str | None = None, session_id: str | None = None) -> ObservationRuntimeConfig:
    ...
    u = {"symbols": list(symbols), "timeframes": list(timeframes)}
    return ObservationRuntimeConfig(
        _commit(baseline_commit), dict(configuration), _sha(configuration),
        f"UNIV-{_sha(u)[:12]}", session_id or _session(),
        runtime_commit or os.environ.get("GITHUB_SHA", UNAVAILABLE_AT_BOUNDARY),
    )


def _direction(x: str | None):
    return {"FAVORABLE": OpportunityDirection.LONG, "UNFAVORABLE": OpportunityDirection.SHORT}.get(x or "")


def _tf(r: OrchestratorResult, tf: str):
    if r.profile is None:
        return None
    m = tuple(x for x in r.profile.timeframes if x.timeframe == tf)
    return m[0] if len(m) == 1 else None


def build_ranking_inputs(r: OrchestratorResult, timeframes: Sequence[str] = REQUIRED_TIMEFRAMES):
    if not (r.dataset and r.profile and r.score and r.decision):
        return ()
    ...
    for tf in timeframes:
        if _tf(r, tf) is None:
            continue
        try:
            if r.dataset.get_timeframe(Timeframe(tf)) is None:
                continue
        except ValueError:
            continue
...
def _reasons(r: OrchestratorResult):
    v = []
    if r.decision:
        v += r.decision.reasons + r.decision.warnings
    if r.score:
        v += r.score.factors + r.score.warnings
    if r.analysis:
        v += r.analysis.signals + r.analysis.warnings
    return tuple(dict.fromkeys(str(x) for x in v if str(x)))


def _critical_indicator_status(r: OrchestratorResult) -> dict[str, dict[str, Any]]:
    result = {}
    ...
    for tf in REQUIRED_TIMEFRAMES:
        ...
        if frame is None:
            result[tf] = {"missing": list(CRITICAL_PROFILE_INDICATORS), "non_finite": list(CRITICAL_PROFILE_INDICATORS), "finite": False}
            continue
        missing = [x for x in CRITICAL_PROFILE_INDICATORS if x not in frame.dataframe.columns]
        non_finite = []
        for name in CRITICAL_PROFILE_INDICATORS:
            if name not in frame.dataframe.columns:
                continue
            value = frame.dataframe[name].iloc[-1]
            try:
                if not bool(__import__("math").isfinite(float(value))):
...
def extract_observation(r: OrchestratorResult, ranked: RankedOpportunity, *, timestamp: datetime):
    ...
    if r.analysis is None:
        missing += ["analysis.market_state", "analysis.strength"]
    if r.profile is None:
        missing += ["profile.trend", "profile.trend_strength", "profile.momentum", "profile.volume_strength", "profile.volatility", "profile.volatility_level", "profile.liquidity"]
    if r.score is None:
        missing.append("raw_score")
    if r.decision is None:
        missing += ["decision", "confidence"]
    tf = _tf(r, ranked.opportunity.timeframe)
    vol = _volume(r, ranked.opportunity.timeframe)
    if tf is None:
        missing.append(f"profile.timeframe:{ranked.opportunity.timeframe}")
    if vol is None:
        missing.append("volume")
    regime = r.profile.market.market_phase if r.profile else None
    liquidity = tf.characteristics.liquidity if tf else None
    if not regime:
...
class PhaseARuntimeObserver:
    def __init__(self, config: ObservationRuntimeConfig, *, orchestrator_factory: Callable[[], Orchestrator], ranker: OpportunityRelativeRanker | None = None):
        self.config = config
        self.factory = orchestrator_factory
        self.ranker = ranker or OpportunityRelativeRanker()
    ...
    def run(self, symbols: Sequence[str], timeframes: Sequence[str]):
        ...
        for symbol in symbols:
            o = self.factory()
            try:
                r = o.run(symbol, list(timeframes))
                status = "SUCCESS"
                ...
                runtime.append({"symbol": symbol, "status": status, "stage": r.statistics.current_stage.value, "error": str(exc), "critical_indicators": _critical_indicator_status(r)})
            else:
        ...
        stamps = [r.dataset.metadata.downloaded_at for _, r in results if r.dataset]
        ts = max(stamps) if stamps else _now()
        ranked = self.ranker.rank(tuple(i for _, r in results for i in build_ranking_inputs(r, timeframes)))
        by_key = {(x.opportunity.symbol, x.opportunity.timeframe, x.opportunity.direction): x for x in ranked}
        journal = SignalJournal()
        records = []
        for symbol, r in results:
            ...
            for tf in timeframes:
                if direction is None:
                    records.append(self._unavailable(symbol, tf, ts, ("directional_raw_strength", "context_score", "composite", "relative_rank", "relative_percentile"), "Decision did not produce a directional signal."))
                    continue
                item = by_key.get((symbol, tf, direction))
                if item is None:
                    records.append(self._unavailable(symbol, tf, ts, (f"profile.timeframe:{tf}",), "Required timeframe was unavailable at the runtime boundary."))
                    continue
                ex = extract_observation(r, item, timestamp=ts)
                if ex.observation:
                    journal = journal.record(SignalJournalEntry(observation=ex.observation))
    ...
    def _unavailable(self, symbol, tf, ts, fields, reason):
    ...
    def _record(self, ex, r, symbol, tf, ts):
        ...
        if ex.observation:
            ...
            rec["observation"] = asdict(ex.observation)
            rec["source_outputs"] = {"analysis": {"market_state": r.analysis.market_state, "strength": r.analysis.strength}, "profile": {"trend": r.profile.market.trend, "trend_strength": r.profile.market.trend_strength, "momentum": r.profile.market.momentum, "volume_strength": r.profile.market.volume_strength, "volatility": r.profile.market.volatility, "volatility_level": r.profile.market.volatility_level, "liquidity": r.profile.market.liquidity}, "score": {"raw_score": r.score.score, "category": r.score.category}, "decision": {"decision": r.decision.decision, "confidence": r.decision.confidence}}
            rec.pop("unavailable_fields", None)
            rec.pop("reason", None)
        return rec
...
def write_artifacts(run: RuntimeObservationRun, artifact_root: Path, universe: Mapping[str, Any]):
    ...
    d = root / "signal-observations" / run.config.session_id
    ...
    write(d / "universe_input.json", universe)
    write(d / "configuration_fingerprint.json", {"algorithm": "sha256", "configuration_fingerprint": run.config.configuration_fingerprint})
    (d / "runtime_results.jsonl").write_text("".join(json.dumps(x, sort_keys=True) + "\n" for x in run.runtime_results), encoding="utf-8")
    (d / "observations.jsonl").write_text("".join(json.dumps(x, sort_keys=True) + "\n" for x in run.records), encoding="utf-8")
    return d
...
def main(argv=None):
    ...
    p.add_argument("--runtime-commit")
    a = p.parse_args(argv)
    symbols, tfs = _universe(REQUIRED_SYMBOLS, REQUIRED_TIMEFRAMES)
    cfg = {"observer_version": OBSERVER_VERSION, "market_source": "BINANCE_API", "symbols": list(symbols), "timeframes": list(tfs), "lookback": {"closed_candles": CLOSED_CANDLE_LOOKBACK, "request_buffer": LOOKBACK_REQUEST_BUFFER}, "execution": {"paper": False, "live": False}, "ranking": {"cohort": ["timeframe", "direction"]}}
    config = create_runtime_config(baseline_commit=a.baseline, symbols=symbols, timeframes=tfs, configuration=cfg, runtime_commit=a.runtime_commit)
    ...
    lookback_source = _ClosedLookbackMarketSource(source, CLOSED_CANDLE_LOOKBACK)
    provider = MarketDataProvider(source=lookback_source)
    run = PhaseARuntimeObserver(config, orchestrator_factory=lambda: build_runtime_orchestrator(provider)).run(symbols, tfs)
    d = write_artifacts(run, Path(a.artifact_root), {"symbols": list(symbols), "timeframes": list(tfs)})
    count = sum(x.get("status") == "OBSERVED" for x in run.records)
    all_runtime_success = all(x.get("status") == "SUCCESS" and x.get("profile_status") == "PASS" and x.get("score_status") == "PRODUCED" and x.get("decision_status") == "PRODUCED" for x in run.runtime_results)
    print(json.dumps({"status": "PASS" if count == 15 and all_runtime_success else "FAIL", "session_id": config.session_id, "observations": count, "closed_candles": CLOSED_CANDLE_LOOKBACK, "artifact_directory": str(d)}, sort_keys=True))
    return 0 if count == 15 and all_runtime_success else 2
