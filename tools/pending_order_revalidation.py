"""Pending-order signal revalidation and cancel/replace state machine.

D5-owned, paper/test-only orchestration.  It consumes canonical signal-time
observation and execution-intent metadata without changing Observer,
Opportunity, Position-FSM, or live-execution contracts.
"""
...
def revalidate_pending_order(
    ...
    *,
    ...
    """Apply D5 stale-signal rules to one pending order."""
    _ensure_aware(now, "now")
    ...
    if same_signal and materially_same:
        return RevalidationResult(RevalidationAction.KEEP)
    if entry_change_pct < policy.minimum_entry_change_pct:
        return RevalidationResult(RevalidationAction.KEEP)
